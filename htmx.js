var htmx = (function() {
   ...
   * @param {(T) => void | Promise<void>} func
   */
   ...
   *
   * @param {Element} elt
   * @param {(extension:HtmxExtension) => void | Promise<void>} toDo
   * @returns void
   */
  async function withExtensions(elt, toDo) {
    await forEach(getExtensions(elt), async function(extension) {
      try {
        await toDo(extension)
      } catch (e) {
        logError(e)
      }
    })
 ...
 */
  ...
  function issueAjaxRequest(verb, path, elt, event, etc, confirmed) {
    ...
    xhr.onload = async function() {
      try {
        ...
        await responseHandler(elt, responseInfo)
   ...
   * @param {HtmxResponseInfo} responseInfo
   */
  async function handleAjaxResponse(elt, responseInfo) {
    const xhr = responseInfo.xhr
    ...
    if (beforeSwapDetails.shouldSwap) {
      ...
      await withExtensions(elt, async function(extension) {
        serverResponse = await extension.transformResponse(serverResponse, xhr, elt)
      })
 ...
 * @property {(text: string, xhr: XMLHttpRequest, elt: Element) => string | Promise<string>} transformResponse
